...
export class Drive {
    ...
    async refreshBrowser() {
        ...
        const fileBrowser = BROWSER.tracker.find(widget => widget.model.driveName === this.name);
        if (fileBrowser) {
...
(function (Private) {
    ...
    function convertToJupyterApi(plotlyObject, fileType, fileName, action, localPath) {
        ...
        if (fileType === 'directory' && action === 'get') {
            ...
            transformedContent = (data.children?.results || []).map((item) => transformItem(item, localPath));
            name = '';
            mimetype = null;
            format = 'json';
        }
